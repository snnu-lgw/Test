...
def Pre_fun(N):
    ...
    now = []
    for i in range(N):
        now.append([i, Quality[i]])  # [课程编号，课程的数据质量和]
...
def fitness1(N, M, unit):
    ...
    return sum(a)/sum(d), sum(b)/sum(d), sum(c) # 除以 Task_num
...
def fitness2(N,M,unit):
    ...
    for j in range(N):
        if (unit[j] == 0 or vis[j]==1):
            Global_variable.b[j] = 0
            continue
        Global_variable.b[j] = int(Global_variable.B * Q_R[j] * unit[j] / Sum)
        now_b = Global_variable.b[j]
        if (now_b == 0):
            continue
        index = Global_variable.lower_bound(mp, now_b/Global_variable.B)  # 横坐标确定
...
def search_answer(N, M, population,flag):
    ...
    for i in range(M):
        if flag == 0:
            ...
            a[i], b[i], c[i] = fitness2(N, M, population[i])
    ...
    f = [new_a[i] + new_b[i] + new_c[i]  for i in range(len(population))]
...
def select(N, M, population,flag):
    ...
    all_fitness = [new_a[i] + new_b[i] + new_c[i]  for i in range(len(population))]
